...
if __name__ == '__main__':
    ...
    def calculate_SWMT_monthly_mean(expt_flux, expt_rho, session_flux, session_rho,
                                    start_time, end_time, frequency, path_output,
                                    filename, lat_north=-59):
        ...
        net_sfc_heating = (
            cc.querying.getvar(
                expt_flux, 'sfc_hflux_from_runoff', session_flux,
                start_time=start_time, end_time=end_time, ncfile='ocean_month.nc') +
            cc.querying.getvar(
                expt_flux, 'sfc_hflux_coupler', session_flux,
                start_time=start_time, end_time=end_time, ncfile='ocean_month.nc') +
            cc.querying.getvar(
                expt_flux, 'sfc_hflux_pme', session_flux, start_time=start_time,
                end_time=end_time, ncfile='ocean_month.nc'))
        frazil_3d_int_z = cc.querying.getvar(
            expt_flux, 'frazil_3d_int_z', session_flux, frequency=frequency,
            chunks={'time': '200MB'})
        # W/m2

        geolon_t = cc.querying.getvar(expt_rho, 'geolon_t', session_rho, n=1,
                                      chunks={'yt_ocean': '200MB'})
        geolat_t = cc.querying.getvar(expt_rho, 'geolat_t', session_rho, n=1,
                                      chunks={'yt_ocean': '200MB'})
        ...
        st_ocean = cc.querying.getvar(expt_rho, 'st_ocean', session_rho, n=1).load()
    ...
    if expt_flux == '01deg_jra55v13_ryf9091_rerun_for_easterlies':
        session_flux = cc.database.create_session()
    else:
        db = (path_db + expt_flux + '.db')
        session_flux = cc.database.create_session(db)
    if expt_rho == '01deg_jra55v13_ryf9091_rerun_for_easterlies':
        session_rho = cc.database.create_session()
    else:
        db = (path_db + expt_rho + '.db')
        session_rho = cc.database.create_session(db)
    ...
    """Load data"""
    ht = cc.querying.getvar(expt_rho, 'ht', session_rho, n=1)
    ht = ht.sel(yt_ocean=slice(-90, -59))
    land_mask = (ht*0).fillna(1)
    yt_ocean = ht.yt_ocean
    xt_ocean = ht.xt_ocean
    area_t = cc.querying.getvar(expt_rho, 'area_t', session_rho, n=1)
    

    start_time = str(year) + '-01-01'
    end_time = str(year) + '-12-31'
    filename_SWMT = ('SWMT_' + expt_flux + '_fluxes_' + expt_rho + '_rho0_' +
                     frequency[0:3:2] + '_' + str(year) + '.nc')
    calculate_SWMT_monthly_mean(expt_flux, expt_rho, session_flux, session_rho,
    ...
    ds.to_netcdf(
        path_output + 'SWMT_in_AABW_formation_region_' + expt_flux +
        '_fluxes_' + expt_rho + '_rho0_' + frequency[0:3:2] + '_' +
